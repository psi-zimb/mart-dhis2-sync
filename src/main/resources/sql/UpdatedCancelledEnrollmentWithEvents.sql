...
FROM (SELECT enrTable.*
      ...
      ON enrTable.date_created :: TIMESTAMP > COALESCE(enrollment_marker.last_synced_date, '-infinity')
          AND category = 'updated_cancelled_enrollment' AND program_name = '%s') AS enrollmentsTable
...
WHERE (enrollmentsTable.status = 'CANCELLED' OR eventsTable.event_program_status = 'CANCELLED')
