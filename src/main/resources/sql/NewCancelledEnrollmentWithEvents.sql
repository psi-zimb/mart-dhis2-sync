...
WHERE enrTable.date_created :: TIMESTAMP > COALESCE((SELECT last_synced_date FROM marker WHERE category = 'new_cancelled_enrollment'
  AND program_name = '%s'),
    '-infinity')
  AND enrTracker.instance_id IS NULL
  AND (enrTable.status = 'CANCELLED') order by enrTable.date_created;